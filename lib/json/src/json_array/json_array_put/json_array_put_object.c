...
** JSON_ARRAY_PUT_OBJECT
** File description:
** Json_array_put_object function
*/
...
{
    return (json_array_put_object_at(ja, value, ja->elements_count));
}

int json_array_put_object_at(json_array_t *ja, json_object_t *value,
    int id)
{
    ...
    if (!ja || id < 0 || id > ja->elements_count)
        return (EXIT_FAILURE);
    je = json_element_create_object(NULL, value);
    if (!je)
        return (EXIT_FAILURE);
